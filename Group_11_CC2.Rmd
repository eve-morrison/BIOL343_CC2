...
positpal::start("Group_11_CC2.Rmd")
```
...
1. **Eve** — 20354956
    + [BIOL343_CC](https://github.com/eve-morrison/BIOL343_CC2.git)
1. 
...
--------|-------------|-------
population_type | Reproductive strategy | none
ovary_width | width of ovary | mm
ovary_height | height of ovary | mm
flower_number | number of flowers |counted
...
```{r import-clean}
library(tidyverse)
setwd("/Users/evemorrison/Documents/343/CC2/data")
cc1_data <- read.csv("A1_Data_Excel.csv") %>% select(poptype, width, height, flonum)
cc1_data$ovary_area <- cc1_data$width*cc1_data$height
names(cc1_data)
print(cc1_data)
```
...
$$\text{width} \times \text{height} = \text{ovary area}$$
$\text{ovary area units are } mm^2$
...
area_means <- aggregate(ovary_area ~ poptype, data=cc1_data, mean)
print(area_means)
```
...
```{r area-table}
library(knitr)
kable(area_means, caption = "Ovary Area Means by Reproductive Strategy")
```
...
library(ggplot2)
Plot1 <- ggplot( aes(x=ovary_area), data=cc1_data) + geom_histogram(binwidth=1.5) + xlab("Ovary Area") + ylab("Number of individuals)") + labs(title="Ovary Area by Reproductive Strategy") + theme_classic() + facet_wrap(~ poptype) 
print(Plot1)
```
...
[Solanum pimpinellifolium](./images/CC2_image.jpg)


Source: 
[Linked .png file](https://onq.queensu.ca/d2l/le/dropbox/1208667/492536/DownloadAttachment?fid=54136532)
